...
ただし, そのままの和訳だと, 長くなる箇所があり, 多少意訳している.

# ゲーム終了か, それとも証明終了か?
コンピュータ支援証明が「本物の」数学とどう違うか気になりますか? それならここがぴったりです!
このゲームでは, 証明アシスタントLean 4と数学ライブラリmathlibの使い方を学べます.
たとえば,
- 帰納法を使った総和公式の証明,
- 写像が全射と右逆元をもつが同値の証明,
- 自然数の列が非可算無限であることの証明,
- 正方行列の空間におけるトレースの特徴付け
などを行います.

インターフェースは簡略化されていますが, 「Editor mode」を有効にすると,
Leanの標準IDEであるVSCodeとほぼ同じ感覚で使えます. スマートフォンやタブレットでは,
デフォルトの「Typewriter mode」のままで, 画面キーボードの自動補完/修正機能をオフにするのがおすすめです.
...
とくに「Babylon」, 「Cantor」, 「Saturn」の各惑星や, tactic・定義のドキュメントが強化されています.
Babylonでは, 今後は ℕ や ℤ の区間での総和を扱い, `Fin n` は使われません.
Saturnのラストには, 多項式的な平方和公式が登場します.

【2025年3月18日】惑星「Quantus」から新惑星「Saturn」が分離. Lunaはサイズアップし, Piazzaも全面的に再設計されました.
...
ただし, そのまま訳すと冗長だったり, 日本で理解しづらい言い回しがあるため, 一部削除・違う表現に書き換えなどがある.
本サイトのリポジトリは以下である.
