...
Statement  (n : ℕ) (hn : 3 ≤ n) : ∑ i ∈ Icc 0 n, (i^3 - 3 * i^2 + 2*i : ℤ ) = ∑ i ∈ Icc 3 n, (i^3 - 3*i^2 + 2*i : ℤ) := by
  ...
  **Robo**: `sum_subset`を使うといいよ：`I₁ ⊆ I₂`で, 
  ...
  Branch
    apply sum_subset
    Hint "**Robo**: いや, それは逆に見えるね. "
    Hint (hidden := true) "**Robo**: まず`symm`で等式の両辺を入れ替えて. "
  symm
  Hint (hidden := true) "**Robo**: いいね. そして`apply sum_subset`. "
  apply sum_subset
  Hint "
    **Robo**: ここでは`Icc_subset_Icc_iff`が役立つはずだよ. 
  "
  ...
  · -- showing that x = 0 or 1 or 2:  see Luna L??
    ...
    Branch
      simp
      intro i h0 h3
      Hint "**Robo**: 仮定から${i}=0$または${i}=1$または${i}=2$が導かれるはず. 
    ...
    have h : i = 0 ∨ i = 1 ∨ i = 2 := by
      ...
      omega
    Hint (hidden := true) "
      **Robo**: 仮定{h}を`obtain h | h | h  := {h}`で3つのケースに分けられるよ. 
    "
    obtain h | h | h  := h
    · rw [h]
      ring
