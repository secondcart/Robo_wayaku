...
Title "であるとき, かつそのときに限り"

Introduction
"
**作業責任者** 私たちは以前, 両方向に動くことができるいくつかのコンベアベルトを持っていました. 最新のマニュアルではこのようなダブルベルトは推奨されていないため, 念のためすべて停止させていました. しかし特定の条件下では安全かもしれませんね? この件についてどう思いますか?
"

Statement (A B : Prop) (mp : A → B) (mpr : B → A) : A ↔ B := by
  Hint "
    **Robo** `A ↔ B` はもちろんLeanでの $A \\iff B$, つまり「同値」を表します.
