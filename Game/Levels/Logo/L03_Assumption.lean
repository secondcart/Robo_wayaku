...
Statement (n : ℕ) (h₁ : 10 > n) (h₂ : 1 < n) (h₃ : n ≠ 5) : 1 < n := by
  Hint "
  **ロボ**: `{n} : ℕ` は、`{n}` が自然数であることを意味します。

    **あなた**: それならなぜ `{n} ∈ ℕ` と書かないの？

    **ロボ**: ここではみんな変なタイプだから…後でゆっくり説明するよ。今はまず問題を解読したいんだ。

    **ロボ**: つまり、`{h₁}`、`{h₂}`、`{h₃}` は、それぞれ `n < 10`、`1 < n`、`n ≠ 5` という仮定の名前だ。証明すべきは `1 < n`。
    ...
    **ロボ**: それを明確に伝えないといけないんだ。`assumption` を使ってみて。"
  assumption
