...
Statement (i : ℕ): (-1 : ℤ)^i  + (-1 : ℤ)^(i+1) = 0 := by
  ...
  Branch
    ...
    Hint "
      **Robo**:  `even_iff_not_odd`を使えば, `¬Odd`を`Even`に変換できます.
      "
  ...
  · rw [Even.neg_pow]
    ...
    · assumption
  · Hint "
      **Robo**:  `odd_iff_not_even`を使えば, `¬Even`を`Odd`に変換できます.
    "
...
Conclusion "今回はFormalosophたちを感心させたようです. 彼らは感心して頷いています.
