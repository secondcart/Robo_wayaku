...
Statement Matrix.one_on_diag_ebasis {n : ℕ} (hn : n > 0) {f : Mat[n, n][ℝ] →ₗ[ℝ] ℝ}
  ...
  **Robo**: こうやってみたらどうかな：
  ...
  suffices h : n * f (E i i) = n * 1 by
    ...
    · assumption
    · Hint  "
      **Robo**: `{hn} : {n} > 0`と`{h} : n = 0`では, `omega`が矛盾を見つけられるはずだよ. 
  ...
  **Robo**: 君の考えを正しく理解しているなら, 今は複数回`trans`を適用したいんだね, 最初に
  ...
  Branch
    ...
    · Hint (hidden := true )"**あなた**: 次に, 関数を総和の中に入れよう. "
      ...
      · Branch
        ...
        Hint (hidden := true) "**Robo**: 今度は`congr`-`ext`? 
        ...
        **あなた**: いや, まず関数を総和の中に入れないとダメだよ. "
        ...
        Hint (hidden := true) "**Robo**: 私の記憶装置によると`eq_on_diag_ebasis`だ. "
        rw [eq_on_diag_ebasis] -- Lvl 5
        assumption
      · Hint (hidden := true) "**Robo**: これは`ebasis_diag_sum_eq_one`みたいだね. "
  ...
  · trans ∑ j : Fin n, f (E i i)
    ...
    · trans ∑ j : Fin n, f (E j j )
      · apply congr_arg
        ext
        Hint (hidden := true) "**Robo**: これはもう見たね. "
        rw [eq_on_diag_ebasis] -- Lvl 5
        assumption
      · trans f 1
        · Hint (hidden := true) "**Robo**: ここで使いたかった結果は`eq_sum_apply_diag_ebasis`だったね. "
          rw [eq_sum_apply_diag_ebasis] -- Lvl 8
          · simp
          · assumption
        · Hint (hidden := true) "**Robo**: `rw [{h₂}]`を試してみて. "
          rw [h₂]
          simp
