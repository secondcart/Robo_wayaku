...
Introduction
"
**Benedictus**: では, 約束していた問題です. `by_contra`を使わずに*直接*証明してみてください.
"
...
Statement (n : ℕ) (h : Odd (n ^ 2)): Odd n := by
  Hint "
    **Robo**: `even_square`の補題に帰着させるのが良いと思います. これは基本的に`Odd (n^2) → Odd n`と言っています.
  ...
  contrapose
  Hint (hidden := true) "**Robo**: 今は`even_iff_not_odd`が役立つかもしれません"
  rw [← even_iff_not_odd]
  rw [← even_iff_not_odd]
  Hint "
    **あなた**: 良さそうです. これで既存の補題`even_square`が使えます! "
  apply even_square
...
Conclusion "**Benedictus**: 素晴らしい! これで十分に準備が整いましたね. "
